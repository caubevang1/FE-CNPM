        ...
        itemRap.danhSachPhim.forEach((itemPhim, iPhim) => {
            danhSachPhim.push({
                label: <div className='flex border-b pb-4'>
                    <div className='mr-4'>
                        <img className='h-full w-24' src={itemPhim.moviePoster} onError={(e) => { e.target.onerror = null; e.target.src = 'https://picsum.photos/75/75' }} />
                    </div>
                    <div>
                        <h2 className='font-bold text-left mb-2 text-sm uppercase'><span className='bg-red-600 p-1 rounded-md text-white text-sm'>{itemPhim.hot === true ? "C18" : "C16"}</span> {itemPhim.movieName}</h2>
                        <div className='grid grid-cols-2 gap-1'>
                            {itemPhim.lstLichChieuTheoPhim?.slice(0, 4).map((itemLichChieu, indexNgayChieu) =>
                                <button onClick={() => navigate(`booking/${itemLichChieu.movieId}`)} key={indexNgayChieu} className="bg-gray-100 hover:bg-gray-300 border-2 text-white font-bold py-2 px-4 rounded">
