    ...
    const renderDanhSachLichChieu = (itemRap) => {
        ...
        itemRap.lichChieuPhim.map((itemLichChieu, iLichChieu) => {
            danhSachLichChieu.push({
                label: <button onClick={() => navigate(`/booking/${itemLichChieu.movieId}`)} className="bg-gray-100 mt-[-1rem] hover:bg-gray-300 border-2 text-white font-bold py-2 px-4 rounded inline-block">
                ...
                {heThongRapChieu?.map(itemHeThongRap => itemHeThongRap.cumRapChieu?.map((itemCumRap, iCumRap) => <div key={iCumRap}>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
                        {itemCumRap.lichChieuPhim?.map((itemLich, iLich) =>
                            <button key={iLich} onClick={() => navigate(`/booking/${itemLich.movieId}`)} className="bg-gray-100 hover:bg-gray-300 border-2 text-white font-bold py-1 rounded">
                                <span className='text-green-500'>
